...
export function createListHandler(listType) {
  ...
  return {
    canExecute: (editor) => {
      return editor.can()[fnName]() || editor.isActive("bulletList") || editor.isActive("orderedList") || editor.isActive("taskList") || editor.isActive("taskItem") || editor.isActive("listItem");
    },
    execute: (editor) => {
      const { state } = editor;
      ...
      if (editor.isActive(listType)) {
        return chain.liftListItem(listItemType).lift("bulletList").lift("orderedList").lift("taskList").selectTextblockEnd();
      }
      const currentListItemType = editor.isActive("taskList") ? "taskItem" : "listItem";
      if (editor.isActive("bulletList") || editor.isActive("orderedList") || editor.isActive("taskList")) {
        const unwrapped = chain.liftListItem(currentListItemType).lift("bulletList").lift("orderedList").lift("taskList");
